...
let rec edgesC startNode endNode cexp i qSet aSet eSet =
    ...
    | Do gc -> 
              let b = B (doneGC gc)
              edgesGC startNode startNode gc i qSet (Set.add b aSet) (Set.add (startNode, b, endNode) eSet)
    | C (c1, c2) -> 
                    let q = N("q" + string i)
    ...
    | Assign (x,y) -> 
                    let a = A cexp
                    (i, qSet, Set.add a aSet, Set.add (startNode, a, endNode) eSet)
    | Skip -> 
            let s = S
            (i, qSet, Set.add s aSet, Set.add (startNode, s, endNode) eSet)
and edgesGC startNode endNode gcexp i qSet aSet eSet = 
    match gcexp with
    | Then (b,c) -> 
                   let q = N("q" + string i)
                   let b1 = B b
                   edgesC q endNode c (i + 1) (Set.add q qSet) (Set.add b1 aSet) (Set.add (startNode, b1, q) eSet)
    | GC (gc1,gc2) -> 
...
let rec edgesCD startNode endNode cexp i qSet aSet eSet d =
    ...
    | Do gc -> 
              edgesGCD startNode startNode gc i qSet (Set.add (B (Not d)) aSet) (Set.add (startNode, B (Not d), endNode) eSet) False
    | C (c1, c2) -> 
                    let q = N("q" + string i)
    ...
    | Assign (x,y) -> 
                    let a = A cexp
                    (i, qSet, Set.add a aSet, Set.add (startNode, a, endNode) eSet, d)
    | Skip -> 
            let s = S
            (i, qSet, Set.add s aSet, Set.add (startNode, s, endNode) eSet, d)
and edgesGCD startNode endNode gcexp i qSet aSet eSet d = 
    match gcexp with
    | Then (b,c) -> 
                   let q = N("q" + string i)
                   let b1 = B(And (b, Not d))
                   edgesCD q endNode c (i + 1) (Set.add q qSet) (Set.add b1 aSet) (Set.add (startNode, b1, q) eSet) (Or(b, d))
    | GC (gc1,gc2) -> 
